<template>
  <div>
    ...
    <el-table size="small" :data="engineData" v-loading="loading">
        ...
        <el-table-column prop="type" label="引擎类型">
            <template slot-scope="scope">
                <span v-if="scope.row.type === 'system'">系统内置</span>
                <span v-if="scope.row.type === 'custom'">用户注册</span>
            </template>
        </el-table-column>
    ...
    <el-dialog title="引擎任务状态" :visible.sync="viewEngineVisible" width="900px" destroy-on-close>
        <el-form :inline="true" :model="engineForm" style="margin-top:-30px">
            <el-col :span="11">
                <el-form-item label="引擎code:">
        ...
        <!--任务列表-->
        <el-table size="small" :data="engineForm.tasks">
            ...
            <el-table-column prop="type" label="任务类型" min-width="180px">
                <template slot-scope="scope">
                    <span v-if="scope.row.sourceType<=1">用例执行</span>
                    <span v-if="scope.row.sourceType>=2">集合/计划执行</span>
                </template>
            </el-table-column>
...
export default {
    ...
    data() {
        return{
            ...
            registerEngineForm: {},
            rules: {
                name: [{ required: true, message: '引擎名称不能为空', trigger: 'blur' }],
            },
            currentUser: ""
        }
    },
    ...
    methods: {
        ...
        getdata(searchParam) {
            this.loading = true
            let url = '/auto_test/engine/list/' + searchParam.page + '/' + searchParam.limit;
            let param = {
        ...
        submitSignEngineForm(confirm, form){
            this.$refs[confirm].validate(valid => {
                if (valid) {
                    let url = '/auto_test/engine/register';
        ...
        // 查看引擎
        viewEngine(engineId){
            let url = '/auto_test/engine/detail/' + engineId;
            this.$get(url, response =>{
                let data = response.data;
                for(let i =0; i<data.tasks.length; i++){
                    data.tasks[i].createTime= timestampToTime(data.tasks[i].createTime);
                    data.tasks[i].index = i+1;
                }
        ...
        deleteEngine(row){
            ...
            .then(() => {
                let url = '/auto_test/engine/delete';
