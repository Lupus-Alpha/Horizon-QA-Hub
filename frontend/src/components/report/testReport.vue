    ...
    <el-table size="small" :data="reportData" v-loading="loading">
        ...
        <el-table-column fixed="right" align="operation" label="操作" width="100px">
            <template slot-scope="scope">
                ...
                <el-button type="text" size="mini" @click="deleteReport(scope.row.id)">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
export default {
    ...
    methods: {
        ...
        getdata(searchParam) {
            // 获取报告
            this.loading = true;
            let url = '/auto_test/report/list/' + searchParam.page + '/' + searchParam.limit;
            let param = {
        ...
        deleteReport(id){
            ...
            .then(() => {
                let url = '/autotest/report/delete/'+id;
