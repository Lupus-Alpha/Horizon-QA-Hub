<template>
  <div>
    ...
    <el-form ref="operationForm" :rules="rules" :model="operationForm" label-width="80px">
        ...
        <el-table :data="operationForm.elements">
            <el-table-column label="元素名称" prop="paramName">
                <template slot-scope="scope">
                    <el-input size="small" style="width: 90%" placeholder="请定义元素名称" v-model="operationForm.elements[scope.$index].paramName"/>
                </template>
            </el-table-column>
            <el-table-column label="元素说明" prop="description">
                <template slot-scope="scope">
                    <el-input size="small" style="width: 90%" placeholder="请输入元素说明" v-model="operationForm.elements[scope.$index].description"/>
                </template>
            </el-table-column>
    ...
    methods: {
        getDetail(param){
            if (param.operationId){ 
                ...
                let url = '/auto_test/operation/detail/'+ param.operationId;
                ...
                if(param.operationType == "assertion"){
                    this.operationForm.data = [
                        ...
                        {paramName:"continue", type:"Boolean", description: "失败后是否继续"},
                    ];
        ...
        cancelAdd(){
            this.$router.push({path: '/baseCenter/operationManage'})
        },
        saveAdd(){
            this.$refs["operationForm"].validate(valid => {
                if (valid) {
                    ...
                    operationForm.elements = JSON.stringify(operationForm.elements);
                    operationForm.data = JSON.stringify(operationForm.data);
                    operationForm.system = operationForm.uiType==='web'?operationForm.uiType:operationForm.system;
                    let url = '/auto_test/operation/add';
        ...
        addElement(){
            this.operationForm.elements.push({paramName:"", description: ""});
        },
        removeElement(index){
            this.operationForm.elements.splice(index, 1);
        },
        deleteAllElement(){
            this.operationForm.elements.splice(0, this.operationForm.elements.length);
        },
