    ...
    <el-table size="small" :data="functionData" v-loading="loading">
        ...
        <el-table-column fixed="right" align="operation" label="操作" width="100px">
            <template slot-scope="scope">
                ...
                <el-button type="text" size="mini" v-if="scope.row.createUser === currentUser & scope.row.from !=='system'" @click="deleteFunction(scope.row.id)">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
export default {
    ...
    methods: {
        ...
        getdata(searchParam) {
            this.loading = true;
            let url = '/auto_test/function/list/' + searchParam.page + '/' + searchParam.limit;
            let param = {
        ...
        // 新增函数
        addFunction(){
            this.$router.push({path: '/baseCenter/function/add'});
        },
        // 查看函数
        viewFunction(row){
            this.$router.push({path: '/baseCenter/function/edit/' + row.id});
        },
        // 删除函数
        deleteFunction(functionId){
            this.$confirm('确定要删除函数吗?', '删除提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            })
            .then(() => {
                let url = '/auto_test/function/delete/'+functionId;
