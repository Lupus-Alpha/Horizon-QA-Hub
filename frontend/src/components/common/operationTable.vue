    ...
    <el-table size="small" :data="operationData" v-loading="loading">
        ...
        <el-table-column fixed="right" align="operation" label="操作" width="100px">
            <template slot-scope="scope">
                ...
                <el-button type="text" size="mini" v-if="scope.row.createUser ===currentUser & scope.row.from !=='system'" @click="deleteOperation(scope.row)">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
export default {
    ...
    methods: {
        ...
        // 查看控件
        viewOperation(row){
            this.$router.push({path: '/baseCenter/operation/' + this.uiType + '/edit/' + row.id});
        },
        // 删除控件
        deleteOperation(row){
