    ...
    <el-table size="small" :data="applicationData" v-loading="loading">
        ...
        <el-table-column fixed="right" align="operation" label="操作" width="100px">
            <template slot-scope="scope">
                ...
                <el-button v-if="showOpt" type="text" size="mini" @click="deleteApplication(scope.row.id)">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
export default {
    ...
    methods: {
      getData(searchParam){
        let url = "/auto_test/application/list/" + searchParam.page + '/' + searchParam.limit;;
        let param = {
      ...
      deleteApplication(id){
        this.$confirm("确定要删除应用吗？", '删除提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
        .then(() => {
            let url = "/auto_test/application/delete/"+id;
      ...
      submitApplication(){
        this.$refs["applicationForm"].validate(valid => {
            if (valid) {
                ...
                let url = "/auto_test/application/save";
