...
""""所有预制操作方法均需要在本文件注册 用于与平台的对接"""


def install_browser_opt(operate_name: str):
    """注册浏览器操作方法"""
    function = None

    def keywords(name): # 传入函数的关键字 判断与当前需要找到的操作名是否一致 一致则将该函数传递回去
        def back(func):
            if name == operate_name:
                nonlocal function #nonlocal 关键字用于在嵌套函数中声明一个变量，该变量来自于外部的非全局作用域。它允许你在内层函数中修改外层（但不是全局作用域）的变量。
                function = func
...
def install_page_opt(operate_name: str):
    """注册页面操作方法"""
    function = None
...
def install_assert_opt(operate_name: str):
    """注册断言方法"""
    function = None
...
def install_relate_opt(operate_name: str):
    """注册关联方法"""
    function = None
...
def install_condition_opt(operate_name: str):
    """注册条件方法"""
    function = None
...
def install_scenario_opt(operate_name: str):
    """注册场景方法"""
    function = None
