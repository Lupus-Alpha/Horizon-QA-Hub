...
public class ZipUtils {
    ...
    public static void compress(String resourcesPath, String targetPath, String targetName) throws Exception {
        // 传入需要打包的文件目录 打包后的保存路径 打包名
        //源文件
        File resourcesFile = new File(resourcesPath); // 初始化被打包文件实例
        //目的文件夹
        File targetFile = new File(targetPath); // 初始化目标目录实例
        if (!targetFile.exists()) { // 如果父目录不存在，创建父目录
            targetFile.mkdirs();
        }
        FileOutputStream outputStream = new FileOutputStream(targetPath+"/"+targetName+".zip"); // 新建io实例
